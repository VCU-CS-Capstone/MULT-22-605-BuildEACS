...
while True:
	...
	#Declaring server name and port number
	serverName = '192.168.8.245'
	serverPort = 12001
	
	#Create socket object using IPv4 and TCP
	client = socket(AF_INET, SOCK_STREAM)
	#Bind address and port and attempt connection
	client.connect((serverName,serverPort))
	#Send rfid number to server
	rfid_new = bytes(rfid_number, 'utf-8')
	client.send(rfid_new)
	
	#Receive response from server
	servResponse = client.recvfrom(2048)
	#Decode from byte format into string
...
#		print ID_list.count(rfid_number)
#		print('User not found.  Access Denied')
