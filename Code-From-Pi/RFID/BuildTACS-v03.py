...
def access_granted():
        GPIO.output(17, False) #Trigger a relay on GPIO 17 (power on state)
...
def machine_turnoff():
        GPIO.output(17, True) #Trigger a relay on GPIO 17 (power off state)
...
def check_user():
    if member_type_list[pos] == 'table_saw' > 0 and relayState == 0:
        log_last = open('/home/pi/Desktop/RFID/user_list.csv' , 'r')
        new_line = (time.strftime('%a %Y-%m-%d,%H:%M:%S') + ',' + rfid_number + ',' +' Approved \n')
        print('First if statement check')
        contents = log_last.readlines()
        contents.insert(0, new_line)
        log_last.close()
        log_last = open('/home/pi/Desktop/RFID/user_list.csv' , 'w')
        log_last.writelines(contents)
        log_last.close()
        access_granted()
        if member_type_list[pos] == 'table_saw' > 0 and relayState == 1:
            log_current = open('/home/pi/Desktop/RFID/user_list.csv' , 'r')
            new_line = (time.strftime('%a %Y-%m-%d,%H:%M:%S') + ',' + rfid_number + ',' +' Approved \n')
            print('Did it make it this far')
            contents = log_current.readlines()
            contents.insert(1, new_line)
            log_current.close()
            log_current = open('/home/pi/Desktop/RFID/user_list,csv' , 'w')
            log_current.writelines(contents)
            log_current.close()
            open('/home/pi/Desktop/RFID/user_list.csv' , 'r')
            if user_list.csv[0] == user_list.csv[1]:
                machine_turnoff()
            if user_list.csv[0] != user_list.csv[1]:
                log_lefton = open('/home/pi/Desktop/RFID/Log/left_on.csv', 'r')
                new_line = (time.strftime('%a %Y-%m-%d,%H:%M:%S') + ',' + rfid_number + ',' + name_list[pos] + ' left machine on \n')
                print(new_line)
                contents = log_lefton.readlines()
                contents.insert(0, new_line)
                log_lefton.close()
                log_lefton = open('/home/pi/Desktop/RFID/Log/left_on.csv' , 'w')
                log_lefton.writelines(contents)
                log_lefton.close()
                access_granted()
...
while True:
    ...
    if ID_list.count(rfid_number) > 0: #Define what a member is
        ...
        if member_type_list[pos] == 'table_saw' > 0 and relayState == 0:
            ...
            check_user()
            relayState = 0
#        if member_type_list[pos] == 'table_saw' > 0 and relayState == 0: #If they are a member, the relay is turned on
#            access_granted()
#            relayState = 1
#        elif member_type_list[pos] == 'table_saw' > 0 and relayState == 1: #If they are a member, they can turn off the machine
#            machine_turnoff()
#            relayState = 0
#    if ID_list.count(rfid_number) == 0: #If they are not a member, they can only ever turn off the machine
#            access_denied()
#            relayState = 0
